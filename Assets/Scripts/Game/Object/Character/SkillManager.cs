...
public class SkillManager : MonoBehaviour 
{
    ...
    public void Create_Skill()
    {
        ...
        for (int i = 0; i < skillButton_array.Length; ++i)
        {
            ...
            {
                Debug.Log(skillButton_array[0].GetPressButton());
                if (skillButton_array[i].img.sprite != image_array[4]) //눌린 버튼의 이미지가 null 이미지가 아니라면
                {
                    SummonsBase _base = new SummonsBase();
                    _base = srt_skillFactory.CreateSkillCharacter(string_array[i], ref skill_array[i]);
                    AttackManager.Object_list.Add(_base);
                    
                    skillButton_array[i].SendMessage("Set_ButtonCoolTime", srt_skillFactory.Get_CoolTime);

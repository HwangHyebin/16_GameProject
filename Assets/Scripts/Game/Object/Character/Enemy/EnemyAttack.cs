    ...
    public sealed override void Execute(Enemy _enemy)
    {
        ...
        if (_enemy.anim.GetCurrentAnimatorStateInfo(0).normalizedTime > 1.0f) //애니메이션이 끝날때, 
        {
            if (startTime == 0)
            {
                ...
                if (_enemy.Get_SkillManager.priority_list != null)
                {
                   
                }
                if (_enemy.Get_Player.player_skills != Player.PLAYER_SKILLS.SHIELD) // 스킬
                {
                    //AttackManager.Instance.AttackHandling("Enemy", "Enemy");
